...
Train = svm_predictor.named_steps['preprocessor'].fit_transform(X_train)
Test = svm_predictor.named_steps['preprocessor'].fit_transform(X_test)
...
# MAGIC     metrics=selection_rate, y_true=y_test, y_pred=svm_predictor.predict(X_test), sensitive_features=X_test['nation']
# MAGIC )
# MAGIC 
...
metric_frame = MetricFrame(
    metrics=metrics, y_true=y_test, y_pred=svm_predictor.predict(X_test), sensitive_features=X_test['nation']
)
...
exponentiated_gradientMitigatedsvm_model = ExponentiatedGradient(
    estimator=svm_predictor,
...
Mitigatedsvm_model = ExponentiatedGradient(
    estimator=svm_predictor,
    constraints=DemographicParity(),
    sample_weight_name="classifier__sample_weight",
)
...
with mlflow.start_run() as run:
    ...
    Mitigatedsvm_model.fit(X_train, y_train, sensitive_features=X_train['nation'])
    
    mlflow.sklearn.log_model(Mitigatedsvm_model, 'EDSP_Mitigated_SVM')
    mlflow.log_params(params)
...
pred = Mitigatedsvm_model.predict(X_test)
pred
...
print(classification_report(y_test, Mitigatedsvm_model.predict(X_test)))
...
# MAGIC %matplotlib inline
# MAGIC selection_rates = MetricFrame(
# MAGIC     metrics=selection_rate, y_true=y_test, y_pred=Mitigatedsvm_model.predict(X_test), sensitive_features=X_test['nation']
# MAGIC )
# MAGIC 
# MAGIC fig = selection_rates.by_group.plot.bar(
# MAGIC     legend=False, rot=0, title="Fraction of Employees Predicted to Resign per National Origin")
...
metric_frame = MetricFrame(
    metrics=metrics, y_true=y_test, y_pred=Mitigatedsvm_model.predict(X_test), sensitive_features=X_test['nation']
)
metric_frame.by_group.plot.bar(
    subplots=True,
    layout=[3, 3],
